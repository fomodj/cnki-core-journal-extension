- # 📚 核心期刊多库标注扩展（人文社科专属增强版）
  ...
  ## 🛠️ 关于数据清洗与项目现状 (Data Cleaning & Project Status)

  本插件的数据集主要来源于万维书刊网及相关公开渠道。在开发和维护过程中，发现学术数据库（如知网、万维网等）的底层数据存在大量的非标准化现象，包括但不限于：
  - 繁简体内外刊名混杂、全角/半角括号及特殊符号（如 `·`、`:`）随意混用。
  - 期刊历史更迭复杂的“原名”、“曾用名”、“即名”在不同页面格式各异。
  - 官方平台自身录入的数据拼写错误。
  
  为了追求完美的 100% 匹配，需要投入海量的时间进行人工核对和规则堆叠。**由于开发者个人精力有限（需要专注于主业与学术科研），目前及未来的原则是“抓大放小”，不再无休止地向脏数据匹配的泥潭中消耗时间。**

  ### 📌 维护策略
  1. **核心功能稳健**：目前版本已针对高频核心期刊进行了深度展平优化，并前置拦截了学位论文、图书、会议等库，日常检索的准确率和页面流畅度已有充分保障。
  2. **边缘 Bug 挂起**：针对极少数生僻刊名、边缘变体的匹配失效，短期内暂不进行主动修复。
  3. **欢迎社区贡献 (PR Welcome)**：如果你在日常使用中发现了错误的匹配，或者你手头有清洗得更干净的、针对特定期刊异名的结构化 JSON 数据，强烈欢迎提交 PR！
